  ...
  return (
    <MarkdownTextPrimitive
      ...
      componentsByLanguage={{
        ...
        mmd: { SyntaxHighlighter: MermaidSyntaxHighlighter },
      }}
    />
  );
};
...
const MermaidSyntaxHighlighter: FC<SyntaxHighlighterProps> = ({
  node,
  components: { Pre, Code },
  code,
}) => {
    ...
    const render = async () => {
      ...
      try {
        ...
        const { svg: renderedSvg, bindFunctions } = await mermaid.render(
          `mermaid-${diagramId}-${currentRender}`,
          source
        );

        if (cancelled || currentRender !== renderCounter.current) return;
        ...
        if (parseError || svgElement.nodeName.toLowerCase() !== 'svg') {
        ...
        const mountedSvg = document.importNode(svgElement, true);
  ...
  if (status === 'error') {
    return (
      <Pre>
        <Code node={node} className="language-mermaid">
          {code}
        </Code>
      </Pre>
    );
  ...
  return (
    <div className="aui-md-mermaid mt-0 mb-2 overflow-x-auto rounded-b-lg border border-border/50 border-t-0 bg-muted/30 p-3">
      {status === 'loading' && (
      ...
      <div
        ref={diagramRef}
        className={cn(
          '[&>svg]:h-auto [&>svg]:max-w-none',
          status !== 'rendered' && 'hidden'
        )}
      />
    </div>
  );
