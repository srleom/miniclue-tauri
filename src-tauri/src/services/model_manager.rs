...
impl ModelManager {
    ...
    /// Return the absolute, user-accessible directory where local models are stored.
    pub fn get_models_storage_dir(&self, app_handle: &AppHandle) -> Result<String, String> {
        let dir = models_root_dir(app_handle)?;
        std::fs::create_dir_all(&dir)
            .map_err(|e| format!("Failed to create models storage dir: {e}"))?;
...
fn models_root_dir(app_handle: &AppHandle) -> Result<PathBuf, String> {
    let app_data = app_handle
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to get app data dir: {e}"))?;
